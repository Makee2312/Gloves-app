          ...
          <>
            ...
            <AnimatePresence mode="wait" initial={false}>
              {activeStep && (
                ...
                >
                  <article className="rounded-xl bg-white shadow-md border border-gray-200 p-4 transition-all hover:shadow-lg">
                    {/* Step Header */}
                    <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-5 border-b border-gray-100 pb-2">
                      ...
                      <time className="text-xs sm:text-sm font-medium italic text-gray-500 sm:text-right">
                        {activeStep.saved_date ?? "-"}
                      </time>
                    </header>

                    {/* Step Data (List / Table Style) */}
                    <div className= {`grid ${activeStep.processType === "qc"? "grid-cols-1" : "grid-cols-2"} gap-4 m-0 p-0`}>
                      {Object.entries(activeStep.data).map(([key, value]) => {
                        ...
                        return (
                          <div
                            key={key}
                            className="rounded-lg mb-2 p-1 overflow-hidden border border-gray-200 bg-gradient-to-br from-white to-gray-50 shadow-sm hover:shadow-md transition-all duration-200"
                          >
                            <div className="flex justify-between items-center px-1 py-3 text-[9px]">
                              <span className="capitalize font-medium text-gray-700">
                                {key.replace(/([A-Z])/g, " $1")}
                              </span>
                              <span className="pl-2 text-[10px] text-gray-900">
                                {value?.toString() ?? "—"}
                              </span>
                            </div>
                          </div>
